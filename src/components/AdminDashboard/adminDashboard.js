...
const AdminDashboard = () => {
            ...
            <div className="bg-white rounded-lg p-6 w-96 max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-md">
                  ...
                  <p className={`font-medium ${selectedvoucher.status === 'Received' ? 'text-green-600' :
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white rounded-lg p-6 w-96 max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-md">
                  <p className="text-sm text-gray-600">Status</p>
                  <p className={`font-medium ${selectedImpactee?.status === 'Approved' ? 'text-green-600' :
  ...
  // Modal Component for Donation History
              ...
              <tbody>
                {donations.map((donation) => (
                  <tr key={donation.id} className="border-b">
                    ...
                    <td className="p-2 border">{"Done"}</td>
                    <td className="p-2 border">
                      {new Date(donation.date).toLocaleDateString()}
                    </td>
                    <td className="p-2 border">{donation.ngo || "N/A"}</td>
                    <td className="p-2 border">{donation.supplier || "N/A"}</td>
  ...
  const RenderUsersList = ({ users, title }) => {
    // Hooks must be inside the component
    const [isHistoryModalOpen, setIsHistoryModalOpen] = useState(false);
    const [donationHistory, setDonationHistory] = useState([]);

    // Open Modal
    const handleShowHistory = (donations) => {
    ...
    // Delete Donor
    const handleDeleteDonor = (userId) => {
      console.log(`Deleting donor with ID: ${userId}`);
      // Add logic to delete donor
    };
      ...
      <div className="bg-white shadow-md rounded-lg p-4 mt-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.donor.id} className="border-b">
                  <td className="p-2">
                    {`${user.donor.firstName} ${user.donor.lastName}`}
                  </td>
                  <td className="p-2">{user.donor.email}</td>
                  <td className="p-2">{user.donor.phone}</td>
                  <td className="p-2 flex space-x-2">
                    {/* History Button */}
                    <button
                      className="text-blue-500 hover:bg-blue-100 p-1 rounded"
                      onClick={() => handleShowHistory(user.donations)}
                    >
                      <History size={16} />
                    </button>
                    ...
                    <button
                      className="text-red-500 hover:bg-red-100 p-1 rounded"
                      onClick={() => handleDeleteDonor(user.donor.id)}
                    >
                      <Trash2 size={16} />
                    </button>
                  </td>
  ...
  const renderNGODetails = (ngo) => (
    <div>
          ...
          {Array.isArray(ngo.causes) && ngo.causes.length > 0 ? (
            ngo.causes.map((cause, index) => (
              <div key={index} className="bg-gray-100 p-3 rounded-lg mb-4">
                ...
                {/* Packages inside each cause */}
                {Array.isArray(cause.packages) && cause.packages.length > 0 ? (
                  <div className="mt-4">
                    <h4 className="text-lg font-semibold mb-2">Packages</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {cause.packages.map((pkg, pkgIndex) => (
                        <div key={pkgIndex} className="bg-white p-3 rounded-lg shadow">
                  ...
                  <p className="mt-2">No packages available for this cause.</p>
                )}
              </div>
            ))
          ) : (
      ...
      {/* Impactees Section */}
      <div className="mt-4">
        <h3 className="text-xl font-semibold mb-2">Impactees</h3>
        {Array.isArray(ngo.impactees) && ngo.impactees.length > 0 ? (
          <ul>
            {ngo.impactees.map((impactee, index) => (
              <li key={index} className="bg-gray-100 p-3 rounded-lg mb-2">
                <p><strong>Name:</strong> {impactee.name}</p>
                <p><strong>Phone:</strong> {impactee.phone}</p>
                <p><strong>CNIC:</strong> {impactee.cnic || "N/A"}</p>
              </li>
            ))}
          </ul>
        ) : (
          <p>No impactees available</p>
        )}
      </div>
    </div>
  );
              ...
              <tr key={supplier._id} className="border-b">
                ...
                <button
                  onClick={() => {
                    ...
                    setSelectedNGO(null); // Clear NGO selection
                    setIsDetailModalOpen(true);
                  }}
                  className="text-blue-500 hover:bg-blue-100 p-1 rounded"
                >
  ...
  const renderDashboardContent = () => {
    switch (activeTab) {
      ...
      case 'donors':
        return <RenderUsersList users={donors} title="Donors List" />;
        ...
        <nav className="p-4">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center space-x-2 p-2 rounded mb-2 ${activeTab === item.tab
  ...
  onClose={() => {
    setIsDetailModalOpen(false);
    setSelectedNGO(null); // Clear NGO selection on close
    setSelectedSupplier(null); // Clear Supplier selection on close
  }}
>
